...
class _MyNoticeVecAnimationState extends State<MyNoticeVecAnimation>
    with TickerProviderStateMixin {//TickerProviderStateMixin适用于多AnimationController的情况。
  ...
  void _startVerticalAni() {
    ...
    _opacityAni1 = Tween<double>(begin: 0.0, end: 1.0).animate(//Tween它主要是弥补 AnimationController 动画值只能为 double 类型的不足,
      CurvedAnimation(
          parent: _controller, curve: Interval(0.0, 0.1, curve: Curves.linear)),//begin 参数 代表 延迟多长时间开始 动画 end 参数 代表 超过多少 直接就是 100% 即直接到动画终点
    );
    ...
    _controller
      ..addStatusListener((status) {
        if (status == AnimationStatus.completed) {//到达动画终点处
          setState(() {
            _nextMassage++;
        ...
        if (status == AnimationStatus.dismissed) {//回到动画起点处
          _controller.forward();
        }
      });
    _controller.forward();
